...
namespace mpd {
	...
	public:
		...
		template<class First, class...Ts, class enabled=std::enable_if_t<!std::is_same_v<std::remove_cv_t<First>,pimpl>>>
		constexpr pimpl(First&& f, Ts&&...vs) noexcept(std::is_nothrow_constructible_v<Impl, Ts...>) { new(buffer())Impl(std::forward<First>(f), std::forward<Ts>(vs)...); }
		...
		template<class T, class enabled = std::enable_if<std::is_assignable_v<Impl, T>>>
		constexpr pimpl& operator=(T&& v) noexcept(std::is_nothrow_constructible_v<Impl, T>) { *buffer() = std::forward<T>(v); return *this; }
