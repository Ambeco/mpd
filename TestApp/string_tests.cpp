...
template<class...Ts>
void test_str_ctor(const wchar_t* expected_value, Ts&&... vals) {
...
template<class F>
void _test_str_mutation(const wchar_t* expected_value, F func) {
...
_test_str_mutation(expected_value, [=](test_string_t& v){op;});
#define test_mutate_and(op, check)  {test_string_t v; op; assert(check);}

template<class F>
void _test_str_big_mutation(const wchar_t* expected_value, F func) {
...
#define test_big_mutation(expected_value, op)  _test_str_big_mutation(expected_value, [=](bigger_string_t& v){op;});

void _test_str_def_ctor() {
	test_string_t str{};
	assert(str.size() == 0);
	assert(*str.data() == 0);
}
...
void test_small_strings() {
	...
	test_str_ctor(L"12345", L"1234567");
	test_str_ctor(L"12345", L"1234567", 7);
	test_str_ctor(L"12345", L"1234567");
	test_str_ctor(L"lmnop", small_lvalue);
	test_str_ctor(L"lmnop", std::move(small_lvalue));
	test_str_ctor(L"xyz,.", init_list);
	test_str_ctor(L"fghij", std_string);
	test_str_ctor(L"ghi", std_string, 1, 3);
	test_str_ctor(L"k", std_string, 5, 5);
	test_str_ctor(L"123", bigger_string_t(L"123"));
	test_str_ctor(L"rstuv", large_lvalue);
	test_str_ctor(L"stuvw", large_lvalue, 1);
	test_str_ctor(L"stu", large_lvalue, 1, 3);
	test_str_ctor(L"w", large_lvalue, 5, 5);
	test_str_ctor(L"ABCDE", c_in_it{ 'A' }, c_in_it{ 'Z' });
